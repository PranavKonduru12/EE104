...
def focus7(event):
    emerg_first_name_field.focus_set()

def focus8(event):
    emerg_last_name_field.focus_set()

def focus9(event):
    emerg_email_field.focus_set()
...
def clear():
    ...
    emerg_first_name_field.delete(0, END)
    emerg_last_name_field.delete(0, END)
    #emerg_phone_field.delete(0, END)
    emerg_email_field.delete(0, END)
...
def first_vaccine():
    ...
    client = Client(
        ...
        "AC950f0d9b4bad5c4d41b010cb4f6a7f1f", "23bce32089d1983251405d149e1a6ce3"
    )
    rows = df.shape[0]
    ...
    from_address = "joeee788@gmail.com"
    password = "SpD5670!"
...
def insert():
    ...
    if (
        ...
        or emerg_first_name_field.get() == ""
        or emerg_last_name_field.get() == ""
        #or emerg_phone_field.get() == ""
        or emerg_email_field.get() == ""
        ...
        sheet.cell(row=current_row + 1, column=12).value = emerg_first_name_field.get()
        sheet.cell(row=current_row + 1, column=13).value = emerg_last_name_field.get()
        sheet.cell(row=current_row + 1, column=14).value = emerg_email_field.get()
...
def register_form():
    ...
    bg = PhotoImage(file=r"BackgroundPicture.png", height=700, width=1000)
    ...
    global email_in
    global address_in
    global emerg_first_name_in
    global emerg_last_name_in
    #global emerg_phone_in
    global emerg_email_in
    ...
    global emerg_first_name_field
    global emerg_last_name_field
    #global emerg_phone_field
    global emerg_email_field
    ...
    emerg_first_name_in = StringVar()
    emerg_last_name_in = StringVar()
    #emerg_phone_in = StringVar()
    emerg_email_in = StringVar()
    ...
    # create a Emergency Contact First Name label
    emerg_first_name = Label(
        root, 
        text="Emergency Contact First Name", bg="black", fg="white", font=("Calibri", 13)
    )

    # create a Emergency Contact Last Name label
    emerg_last_name = Label(
        root, text="Emergency Contact First Name", bg="black", fg="white", font=("Calibri", 13)
    )

    # create a Emergency Contact Email label
    emerg_email = Label(
        root, 
        text="Emergnecy Contact Email", bg="black", fg="white", font=("Calibri", 13)
    )
    ...
    emerg_first_name.grid(row=8, column=0)
    emerg_last_name.grid(row=9, column=0)
    emerg_email.grid(row=10, column=0)
    ...
    emerg_first_name_field = Entry(root, textvariable=emerg_first_name_in) # For emergency contact
    emerg_last_name_field = Entry(root, textvariable=emerg_last_name_in)
    #emerg_phone_field = Entry(root, textvariable=emerg_phone_in)
    emerg_email_field = Entry(root, textvariable=emerg_email_in)
    ...
    emerg_first_name_field.bind("<Return>", focus8)

    emerg_last_name_field.bind("<Return>", focus9)

    #emerg_phone_field.bind("<Return>", focus10)

    #emerg_email_field.bind("<Return>", focus10)
    ...
    emerg_first_name_field.grid(row=8,column=1, ipdax="100")
    emerg_last_name_field.grid(row=9,column=1, ipdax="100")
    #emerg_phone_field.grid(row=10,column=1, ipdax="100")
    emerg_email_field.grid(row=10,column=1, ipdax="100")
...
def register():
    ...
    # refrenced above
    bg = PhotoImage(file=r"sjsuTower2.png", height=420, width=300)
...
def excel2():
    ...
    # w
    # rite given data to an excel spreadsheet
...
def main_account_screen():
    ...
    bg = PhotoImage(file=r"towerSjsu.png", height=400, width=900)
...
main_account_screen() # Start from the main account screen
