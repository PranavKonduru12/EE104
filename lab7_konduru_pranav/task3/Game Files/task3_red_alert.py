...
def update():
    ...
    if (game_complete or game_over) and keyboard.space:      # try again feature when pressing space key
        stars = []
        current_level = 1
        game_complete = False
        game_over = False
...
def create_stars(colors_to_create): 
    ...
    for color in colors_to_create: 
        star = Actor(color + "-snowflake")      # Changed actor to snowflakes instead of stars
...
def animate_stars(stars_to_animate):
    for star in stars_to_animate:
        random_speed_adjustment = random.randint(0, 2)      # Used randint to modify duration (Need for speed)
